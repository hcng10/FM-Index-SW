...
inline void writeSAMVal(uint64_t &bytes, char *buffer, void *a, uint64_t a_bytes){
    ...
    for (int8_t i = 1; i<sizeof(tmp_buffer); i++){
        if (tmp_buffer[i] != 0){
...
inline void setSAMOptional(uint64_t &bytes, char *buffer, read_t & read, uint32_t & suppress_aligned){
...
inline void setSAMFlag(uint64_t &bytes, char *buffer, read_t & read){
...
uint16_t getChrsIdx(uint32_t pos, std::vector<chr_t> &chrs){
...
void writeSAMFields(uint64_t &bytes, 
    ...
    // 1. QNAME
    writeSAMCStr(bytes, buffer, (void *)(read.at_data.c_str()+1), read.at_data.size()-1);
    writeSAMTab(bytes, buffer);
...
void convertSAM(FILE *fp_sam, std::vector<read_t> &reads, uint32_t *sai, char *buffer, std::vector<chr_t> &chrs){
    ...
    for (uint32_t i = 0; i < reads.size(); i++) {
        ...
        if (read.is_b_align){
            for (uint32_t j = 0; j < read.b_high - read.b_low && j < ALIGN_REPORT_NUM; j++){
                aligned_sai_b[j] = sai[read.b_low + j] + 1;
                aligned_num_b++;
            }
            suppress_aligned += (read.b_high - read.b_low) > ALIGN_REPORT_NUM ? 
                read.b_high - read.b_low - ALIGN_REPORT_NUM: 0;

        }

        if (read.is_f_align){
            for (uint32_t j = 0; j < read.f_high - read.f_low && j < ALIGN_REPORT_NUM; j++){
                aligned_sai_f[j] = sai[read.f_low + j] + 1;
                aligned_num_f++;
            }                
            suppress_aligned += (read.f_high - read.f_low) > ALIGN_REPORT_NUM ? 
                            read.f_high - read.f_low - ALIGN_REPORT_NUM: 0;
        }
